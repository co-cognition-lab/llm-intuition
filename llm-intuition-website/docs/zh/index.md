...
[**LLM 直觉盲区探索**](/zh/main) 是这个版图的第一个锚点——四个直觉子类型、构成性退化、人优先协议，为整个版图提供了理论地基。[**🪝 LLM 钩子实验**](/zh/experiments/llm-hook) 是一次实证检验：在网页中嵌入隐藏指令，实测 7 个 LLM 是否会遵从——GPT-4o、Claude、Gemini 全数失败，直接用行为证明了 B1 退化路径的真实性。[**竞争格局推演**](/zh/competition) 是理论向应用的一次延伸：用直觉子类型的框架去分析全球 LLM 赛道，同时也是一个可检验的断言——留给时间去验证。报告内含 6 个核心判断、4 条路线的退化风险排序、8 个未来情景的概率推演、以及一个中国路线的专项分析。如果只想花 3 分钟，可以从"核心判断"和"情景概率总表"开始。

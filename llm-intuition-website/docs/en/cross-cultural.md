---
title: 'Cross-Cultural Assessment'
description: 'WEIRD premise audit, cross-cultural differences across four subtypes, cultural adaptation of the Human-first Protocol.'
---
...
1. **Do not overturn the existing framework**—the main thread of "cost externality → Embodiment → decreasing LLM accessibility" in the current framework still holds in cross-cultural contexts, but the specific manifestations of each subtype may vary across cultures.
2. **Identify structural shifts**—certain cultural dimensions may cause Complementarity Map determinations to shift from ⚠️ to ❌ or from ❌ to ⚠️.
3. **Honestly mark uncertainty**—areas lacking cross-cultural empirical evidence are explicitly labeled; no speculative gaps are filled.
...
- UAI → determines AI adoption speed control: "conservative speed limit" (high UAI) / "standard speed limit" (low UAI)
```

---

## 7. Research Priority Recommendations
...
[South Asia] LLM has no body + no guru authority → no moral intuition + no religious authority certification → analysis is reachable but judgment is unreachable for different reasons
```
...
1. **Whether East Asian "reading the air" is more difficult to substitute with AI than Western "reading people"**—theoretical inference is "yes," but lacks direct cross-cultural AI test data.
2. **Whether Conceptual Type intuition cultivated by drill-based education is more easily substitutable by LLM**—based on theoretical inference from education model comparison, no direct experimental evidence.
3. **Whether high uncertainty avoidance cultures naturally slow degradation**—theoretically protective, but may be offset by "group adoption pressure"—net effect unknown.
4. **Whether "collective intuition" in Ubuntu culture exists**—conceptually reasonable, but lacks cognitive science empirical evidence.
5. **Acceptance of AI moral assistance in Islamic contexts**—highly dependent on specific legal schools and contemporary religious scholars' positions—requires field research.

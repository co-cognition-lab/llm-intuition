...
这个版图的中心工程是 **Co-Cognition 全景图**（项目 2）：什么样的知识领域因方法论范式不兼容而至今少有交叉？co-cognition 机制在哪些维度上能提供独特的拓展价值？这是一套跨学科的分类学与评分体系，正在从 108 个候选域逐步收敛。

**LLM 直觉盲区探索**（项目 1）是这个版图的第一个锚点——四个直觉子类型、构成性退化、人优先协议，为整个版图提供了理论地基。**竞争格局推演**（项目 4）是第一个锚点的一个延伸：用直觉子类型的框架去分析全球 LLM 赛道，同时也是一个可检验的断言——留给时间去验证。

在绘制版图的过程中，我们发现一个跨象限的空白：**危机与极限情境下的人类-AI 认知协作**。现有分类学没能覆盖这个维度——**危机认知协作**（项目 3）正是为了填补它。
